...
def clean_column(series, keywords_list=None, keywords_dict=None, parallel=False, n_jobs=-1, chunk_size=1000):
    """
    ...
    def process_chunk(chunk):
        ...
        if keywords_dict:
            for key, value in keywords_dict.items():
                chunk = chunk.str.replace(rf"\b{re.escape(key)}\b", f"{value}", regex=True)
    ...
    if parallel:
        ...
        chunks = [series.iloc[i * chunk_size:(i + 1) * chunk_size] for i in range(num_chunks)]

        # Parallel processing
        with tqdm_joblib(tqdm(desc="Processing chunks", total=len(chunks))) as progress_bar:
...
# old version of clean_column
# def clean_column(series, keywords_list=None, keywords_dict=None):
#     """
#     Cleans a pandas Series with custom replacements and text preprocessing.
...
#     """
#     # Step 1: Use recordlinkage.clean for basic cleaning
#     cleaned_series = clean(
#         series,
#         lowercase=True,
#         replace_by_none='[^ \\-\\_A-Za-z0-9]+',  # Remove non-alphanumeric characters
#         replace_by_whitespace='[\\-\\_]',        # Replace "-" and "_" with spaces
...
#     # Step 2: Replace keywords from keywords_list with a single space
#     if keywords_list:
#         for keyword in keywords_list:
#             cleaned_series = cleaned_series.str.replace(rf"\b{re.escape(keyword)}\b", " ", regex=True)

#     # Step 3: Replace keywords from keywords_dict with specific strings
#     if keywords_dict:
#         for key, value in keywords_dict.items():
#             cleaned_series = cleaned_series.str.replace(rf"\b{re.escape(key)}\b", f" {value} ", regex=True)
...
def winsorize(df, lower_percentile=1, upper_percentile=99):
    ...
    for col in df.columns:
        if not is_numeric_dtype(df[col]) or isinstance(df[col].dtype, pd.CategoricalDtype):  # Skip non-numeric or cardinality categorical variables
            continue
...
def log_transform(df, y, threshold=10):
    ...
    for col in df.columns:
        if not is_numeric_dtype(df[col]) or isinstance(df[col].dtype, pd.CategoricalDtype):  # Skip non-numeric or low-cardinality variables
            continue
        ...
        if (df[col] > 0).all() and ((col_magnitude / y_magnitude > threshold) or (abs(col_skewness) > 2 or col_kurtosis > 9)):
            log_transformed_columns.append(col)
            df[f"ln_{col}"] = np.log(df[col])  # Apply log(x) assuming strictly positive values
...
def create_aggregate_variable(df, levels, var, func):
    ...
    agg_col_name = f'{"_".join(levels)}_{func.__name__}_{var}' if callable(func) else f'{"_".join(levels)}_{func}_{var}'
...
class NgramsOverlap(BaseIndexAlgorithm):
    ...
    def _ngrams_overlap(self, df_queries, df_candidates, query_column, candidate_column, n):
        """
        ...
        vectorizer = CountVectorizer(analyzer='char', ngram_range=(n, n), binary=True, dtype=np.uint8)
...
def determine_entity(df, column, string_list):
    ...
    contains_any = pd.concat(
        [df[column].str.lower().str.contains(fr'\b{s}\b', na=False, regex=True) for s in string_list],
        axis=1
    )
