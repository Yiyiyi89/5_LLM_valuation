...
def analyze_moat(metrics: list) -> dict[str, any]:
    ...
    historical_roes = [
        m.return_on_equity for m in metrics if m.return_on_equity is not None
    ]
    historical_roics = [
        m.return_on_invested_capital
        for m in metrics
        if hasattr(m, "return_on_invested_capital")
        and m.return_on_invested_capital is not None
    ]
    ...
    if len(historical_roes) >= 5:
        ...
        if roe_consistency >= 0.8:  # 80%+ of periods with ROE > 15%
            ...
            moat_score += 1
            reasoning.append(
                f"Good ROE performance: {high_roe_periods}/{len(historical_roes)} periods >15%"
            )
        else:
            reasoning.append(
                f"Inconsistent ROE: only {high_roe_periods}/{len(historical_roes)} periods >15%"
            )
    else:
    ...
    if len(historical_margins) >= 5:
        ...
        if (
            avg_margin > 0.2 and recent_avg >= older_avg
        ):  # 20%+ margins and stable/improving
            moat_score += 1
            reasoning.append(
                f"Strong and stable operating margins (avg: {avg_margin:.1%}) indicate pricing power moat"
            )
        elif avg_margin > 0.15:  # At least decent margins
            reasoning.append(
                f"Decent operating margins (avg: {avg_margin:.1%}) suggest some competitive advantage"
            )
        else:
            reasoning.append(
                f"Low operating margins (avg: {avg_margin:.1%}) suggest limited pricing power"
            )
    ...
    if len(metrics) >= 5:
        ...
        if len(asset_turnovers) >= 3:
            if any(
                turnover > 1.0 for turnover in asset_turnovers
            ):  # Efficient asset use
                moat_score += 1
                reasoning.append(
                    "Efficient asset utilization suggests operational moat"
                )
    ...
    if len(historical_roes) >= 5 and len(historical_margins) >= 5:
        ...
        roe_variance = sum((roe - roe_avg) ** 2 for roe in historical_roes) / len(
            historical_roes
        )
        ...
        margin_variance = sum(
            (margin - margin_avg) ** 2 for margin in historical_margins
        ) / len(historical_margins)
        margin_stability = (
            1 - (margin_variance**0.5) / margin_avg if margin_avg > 0 else 0
        )
        ...
        if (
            ...
            moat_score += 1
            reasoning.append(
                f"High performance stability ({overall_stability:.1%}) suggests strong competitive moat"
            )
    ...
    return {
        ...
        "max_score": max_score,
        "details": (
            "; ".join(reasoning) if reasoning else "Limited moat analysis available"
        ),
    }
...
def calculate_owner_earnings(financial_line_items: list) -> dict[str, any]:
    ...
    if not all([net_income is not None, depreciation is not None, capex is not None]):
        ...
        return {
            "owner_earnings": None,
            "details": [f"Missing components: {', '.join(missing)}"],
        }
        ...
        try:
            ...
            if all(
                [
                    ...
                    current_liab_current,
                    current_assets_previous,
                    current_liab_previous,
                ]
            ):
                ...
                details.append(
                    f"Working capital change: ${working_capital_change:,.0f}"
                )
        except:
    ...
    owner_earnings = (
        net_income + depreciation - maintenance_capex - working_capital_change
    )

    # Sanity checks
    if (
        owner_earnings < net_income * 0.3
    ):  # Owner earnings shouldn't be less than 30% of net income typically
        details.append(
            "Warning: Owner earnings significantly below net income - high capex intensity"
        )

    if (
        maintenance_capex > depreciation * 2
    ):  # Maintenance capex shouldn't typically exceed 2x depreciation
        details.append(
            "Warning: Estimated maintenance capex seems high relative to depreciation"
        )
...
def estimate_maintenance_capex(financial_line_items: list) -> float:
    ...
    for item in financial_line_items[:5]:  # Last 5 periods
        ...
        if (
            hasattr(item, "depreciation_and_amortization")
    ...
    latest_capex = (
        abs(financial_line_items[0].capital_expenditure)
        if financial_line_items[0].capital_expenditure
        else 0
    )
    ...
    if len(capex_ratios) >= 3:
        ...
        latest_revenue = (
            ...
            if hasattr(financial_line_items[0], "revenue")
            and financial_line_items[0].revenue
            else 0
        )
...
def calculate_intrinsic_value(financial_line_items: list) -> dict[str, any]:
    ...
    if not shares_outstanding or shares_outstanding <= 0:
        return {
            "intrinsic_value": None,
            "details": ["Missing or invalid shares outstanding data"],
        }
    ...
    if len(historical_earnings) >= 3:
        ...
        if oldest_earnings > 0:
            ...
            historical_growth = max(
                -0.05, min(historical_growth, 0.15)
            )  # Cap between -5% and 15%
            conservative_growth = (
                historical_growth * 0.7
            )  # Apply 30% haircut for conservatism
        else:
    ...
    stage2_growth = min(
        conservative_growth * 0.5, 0.04
    )  # Stage 2: half of stage 1, cap at 4%
    ...
    return {
        ...
        "assumptions": {
            ...
            "historical_growth": (
                conservative_growth if "conservative_growth" in locals() else None
            ),
        },
        "details": details,
    }
...
def analyze_book_value_growth(financial_line_items: list) -> dict[str, any]:
    ...
    if len(book_values) >= 3:
        ...
        for i in range(len(book_values) - 1):
            if (
                book_values[i] > book_values[i + 1]
            ):  # Current > Previous (reverse chronological)
                growth_periods += 1
        ...
        if growth_rate >= 0.8:  # 80% of periods show growth
            score += 3
            reasoning.append(
                "Consistent book value per share growth (Buffett's favorite metric)"
            )
...
def analyze_pricing_power(financial_line_items: list, metrics: list) -> dict[str, any]:
    ...
    if len(gross_margins) >= 3:
        ...
        recent_avg = (
            sum(gross_margins[:2]) / 2 if len(gross_margins) >= 2 else gross_margins[0]
        )
        older_avg = (
            sum(gross_margins[-2:]) / 2
            if len(gross_margins) >= 2
            else gross_margins[-1]
        )
    ...
    if gross_margins:
        ...
        if avg_margin > 0.5:  # 50%+ gross margins
            score += 2
            reasoning.append(
                f"Consistently high gross margins ({avg_margin:.1%}) indicate strong pricing power"
            )
        elif avg_margin > 0.3:  # 30%+ gross margins
            score += 1
            reasoning.append(
                f"Good gross margins ({avg_margin:.1%}) suggest decent pricing power"
            )
    ...
    return {
        ...
        "details": (
            "; ".join(reasoning)
            if reasoning
            else "Limited pricing power analysis available"
        ),
    }
